...
dnd5esheets/client/openapi.json: dnd5esheets/api.py dnd5esheets/schemas.py
	...
	cd dnd5esheets && poetry run uvicorn dnd5esheets.app:app&
	@sleep 3  # Sorry dad
	curl -s http://localhost:8000/openapi.json > dnd5esheets/client/openapi.json
	kill $$(lsof -i tcp:8000 | grep -v PID | head -n 1 | awk '{ print $$2 }')
